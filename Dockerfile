...
RUN apt update
RUN apt -y install bc bison build-essential ccache curl \
                   flex g++-multilib gcc-multilib git gnupg \
                   ...
                   zlib1g-dev openjdk-8-jdk python procps
...
ARG UID
ARG GID
# user with host user id/gid and no home dir
RUN groupadd -g $GID buildgroup && \
    useradd -u $UID -g $GID -M builduser
